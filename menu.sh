...
show_header() {
  printf '%s\n' \
    ...
    'Aucune action systeme reelle n est autorisee par ce menu.' \
    ''
}
...
while true; do
  show_header
  cat <<'EOF'
1) Diagnostic architecture / securite
2) Afficher le plan complet HOST -> KVM -> VM_DEVOPS -> BACKUP
3) Afficher les gates de securite
0) Quitter
EOF
